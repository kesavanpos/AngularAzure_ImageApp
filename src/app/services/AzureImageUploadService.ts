  ...
  uploadImage(file: File): Observable<any> {
    ...
    return this.http.put(url, file, { headers, responseType: 'text' }).pipe(
      catchError((error) => {
        ...
        return throwError('Failed to upload the image. Please check the console for more details.');
      })
    );
  }
  ...
  getImages(): Observable<BlobImage[]> {
    ...
    return this.http.get(url, { responseType: 'text' }).pipe(
      ...
      catchError((error) => {
        ...
        return throwError('Failed to fetch images. Please check the console for more details.');
      })
    );
  }
